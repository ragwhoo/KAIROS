...
export function Sidebar() {
  ...
  // Desktop animate content visibility
  useLayoutEffect(() => {
    if (isMobile) return
    if (sidebarOpen) {
  ...
  return (
    <>
      ...
      <aside
        ref={asideRef}
        className={cn(
          "fixed left-0 top-0 z-30 flex h-full flex-col border-r border-border bg-background overflow-hidden",
          isMobile && "w-[280px]",
      ...
      >
        ...
        <nav className="flex-1 space-y-1 px-2 py-4 overflow-hidden">
          {navItems.map((item) => (
            <SidebarItem key={item.href} icon={item.icon} label={item.label} href={item.href} collapsed={!isMobile && !sidebarOpen} />
          ))}
        </nav>
